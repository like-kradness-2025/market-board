...
```bash
uv run python send.py --no-discord
```

Discord webhook は環境変数またはファイルで指定する。

```bash
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/... uv run python send.py
DISCORD_WEBHOOK_FILE=~/webhook/MarketBoard uv run python send.py
```

DB の場所を変更する場合:

```bash
COINALYZE_DB=/path/to/coinalyze_v2.db uv run python send.py
```
